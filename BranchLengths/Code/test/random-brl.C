...
Vector3d multivariateGamma3D(Vector3d mu,Matrix3d vc,mt19937_64& rng, double& logdensity, bool verbose, ofstream& par3D)
{
  ...
  if(verbose)
    cout << "T3: " << bl[2] << endl;
  logdensity += alpha1*log(lambda1) + (alpha1-1)*log(bl[0]) - lambda1*bl[0] - lgamma(alpha1)
    +           alpha2*log(lambda2) + (alpha2-1)*log(bl[1]) - lambda2*bl[1] - lgamma(alpha2)
    +           alpha3*log(lambda3) + (alpha3-1)*log(bl[2]) - lambda3*bl[2] - lgamma(alpha3);
