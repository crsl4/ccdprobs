...
// 1.0.6 --- 21 June, 2013 --- fixed memory leak in RootedTree constructor
//       --- 16 July, 2013 --- fixed error in readTranslateTable for Windows files
...
void usage()
{
  cerr << "Usage: ccdprobs <--out|-o> root-name-for-output-files [--extra] extra_tree_file_input <--in|-i> infile" << endl << endl;
...
void initialize(Files& f,int argc,char* argv[])
{
  if ( argc==1 )
    usage();
...
void writeOutput(Files& f, vector<RootedTree>& trees, vector<int>& counts, int sampleSize, map<Clade,int>& cladeCount, map<CladePair,int>& pairCount, vector<int>& taxaNumbers, vector<string>& taxaNames)
{
...
int main(int argc, char* argv[])
{
  ...
  cerr << "taxaNumbers.size() = " << taxaNumbers.size() << ", taxaNames.size() = " << taxaNames.size() << endl;
  ...
  // Write output files
  writeOutput(f,trees,counts,sampleSize,cladeCount,pairCount,taxaNumbers,taxaNames);
