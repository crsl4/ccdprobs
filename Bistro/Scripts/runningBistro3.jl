...
function runBistro(stem)
    ...
    try
        ##run(`rm -f bistro-6-$stem.log`)
        out = readstring(`../../Code/bistro/bistro -f ../../Data/datasets/$stem.fasta -o bistro-6-$stem -r 1000 -b 1000`);
        f = open("bistro-6-$stem.log","w")
        write(f,out)
        close(f)
    catch
...
function runBistroFixT(stem,tree)
    ...
    try
        ...
        out = readstring(`../../Code/bistro/bistro -f ../../Data/datasets/$stem.fasta -o bistroFixT-6-$stem -r 1000 -b 1000 -t "$tree"`);
        f = open("bistroFixT-6-$stem.log","w")
        write(f,out)
        close(f)
    catch
