...
## Claudia June 2016
## modified to run only one ntax, instead of all, so that bistroOneRep.pl
## controls everything
## run as Rscript simulateDataStudy.r rep seed ntax
...
pruned.tree$edge.length <- rep(0.1,nl) ## cheating to make bl comparable
...
write(write.tree(pruned.tree),file="true-trees.txt", append=TRUE)
