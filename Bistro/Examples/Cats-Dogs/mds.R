...
d = matrix(scan("bistro4---0-249.treeBL.distances"),100,100)
out = cmdscale(d,k=2)
...
trees = data.frame(top=temp.trees)
trees = trees %>%
  mutate(rank=6)
n = nrow(tab)
for ( i in 1:6 )
...
df = data.frame(x=out[,1],y=out[,2],w=exp(-200*d[,100]),top=trees$top,rank=trees$rank)
...
df2 = data.frame(dist=d[,100],mds.dist=sqrt(df$x^2+df$y^2),top=df$top,rank=df$rank)
