...
mcmc_out = read.table("bistro024-2.par",header=FALSE)
# remove burnin
...
bad.mean = apply(mcmc_out[-(1:100),-1],2,mean)
bad.sd = apply(mcmc_out[-(1:100),-1],2,sd)

## read in IS sample
source("../../Scripts/readBistro.R")
bistro = readBistro("bistro024-2")
b = bistro %>%
...
ggplot(mcmc_out[-(1:100),],aes(x=1:1000,y=logl)) + geom_point()

ggplot(mcmc_out,aes(x=-99:1000)) +
#  geom_point(aes(y=logl))
